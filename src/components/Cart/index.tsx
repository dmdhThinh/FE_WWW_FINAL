...
import { fetchCartItems, clearCart, selectCartItems, selectCartLoading, selectCartInitialized } from "@/redux/features/cart-slice";
...
const Cart = () => {
  ...
  useEffect(() => {
    // Only fetch cart items if not already initialized
    if (!isInitialized) {
          ...
          <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
            <div className="flex flex-wrap items-center justify-between gap-5 mb-7.5">
              ...
              <button
                ...
                disabled={isLoading}
                className="text-blue hover:text-blue-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
              >
                {isLoading ? 'Đang xóa...' : 'Xóa toàn bộ giỏ hàng'}
              </button>
            </div>
