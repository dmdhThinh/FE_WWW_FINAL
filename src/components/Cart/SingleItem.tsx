...
const SingleItem: React.FC<SingleItemProps> = ({ item }) => {
  ...
  const handleRemoveFromCart = async () => {
    if (item.productId) {
      try {
        await dispatch(removeFromCart({ productId: item.productId })).unwrap();
      } catch (error) {
  ...
  const handleUpdateQuantity = async (newQuantity: number) => {
    if (newQuantity < 1 || !item.productId) return;
  ...
  return (
    <div className="flex items-center border-t border-gray-3 py-5 px-7.5">
          ...
          <div className="w-full flex items-center gap-5.5">
            ...
            <div>
              <h3 className="text-dark ease-out duration-200 hover:text-blue">
                <a href="#"> {item.productName || "Product"} </a>
              </h3>
            </div>
          </div>
        </div>
      ...
      <div className="min-w-[180px]">
        <p className="text-dark">${item.price || 0}</p>
      </div>
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">
          ${(item.subtotal || (item.price || 0) * quantity).toFixed(2)}
        </p>
      </div>
