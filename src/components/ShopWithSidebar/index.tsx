...
const ShopWithSidebar = () => {
  ...
  // Parse URL parameters and create filters
  const parseUrlParams = useCallback((): ProductFilters => {
    ...
    if (page) params.page = parseInt(page, 10) - 1; // Convert to 0-based index
    ...
    if (categoryIds) {
      ...
      params.categoryIds = categoryIds.split(',').map(id => parseInt(id.trim(), 10)).filter(id => !isNaN(id));
    }
  ...
  // Product filters state from URL
  const [filters, setFilters] = useState<ProductFilters>(parseUrlParams);

  // Search input state (separate from filters for debounce)
  ...
  useEffect(() => {
    ...
    setSearchInput(newFilters.name || "");
  }, [parseUrlParams]);
  ...
  const sortOptions = [
    { label: "Sản phẩm mới nhất", value: "createdAt", order: "desc" },
    { label: "Giá: Thấp đến Cao", value: "price", order: "asc" },
    { label: "Giá: Cao đến Thấp", value: "price", order: "desc" },
    { label: "Tên: A đến Z", value: "name", order: "asc" },
    { label: "Tên: Z đến A", value: "name", order: "desc" },
  ];
  ...
  const updateUrlParams = useCallback(
    (newFilters: ProductFilters) => {
      ...
      if (newFilters.page && newFilters.page > 0) {
        params.set("page", (newFilters.page + 1).toString()); // Convert to 1-based index for URL
      }
      if (newFilters.size && newFilters.size !== 12) {
        params.set("size", newFilters.size.toString());
      }
      if (newFilters.name) {
        params.set("name", newFilters.name);
      }
      // Support multiple categories - prefer categoryIds over categoryId
      if (newFilters.categoryIds && newFilters.categoryIds.length > 0) {
        params.set("categoryIds", newFilters.categoryIds.join(','));
        // Remove single categoryId if categoryIds is present
        params.delete("categoryId");
      } else if (newFilters.categoryId) {
        params.set("categoryId", newFilters.categoryId.toString());
        // Remove categoryIds if single categoryId is present
      ...
      // Avoid useless router pushes that can trigger repeat renders
  ...
  const handleCleanAll = () => {
    const defaultFilters: ProductFilters = { page: 0, size: 6 };
  ...
  // stable because handleFiltersChange is now stable
  ...
  return (
    <>
      <Breadcrumb
        ...
        pages={["cửa hàng", "/", "cửa hàng với thanh bên"]}
      />
          ...
          <div className="flex gap-7.5">
            ...
            >
              <button
                ...
                aria-label="nút chuyển đổi thanh bên sản phẩm"
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Bộ lọc:</p>
                      <button onClick={handleCleanAll} className="text-blue">
                        Xóa tất cả
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="bg-white shadow-1 rounded-lg p-5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-4 py-2 border border-gray-3 rounded-lg focus:outline-none focus:border-blue text-sm"
                      />
                      {searchInput && (
                        <button
                          onClick={() => {
                          ...
                          className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-4 hover:text-dark"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
                  ...
                  <CategoryDropdown
                    ...
                    onCategoryChange={(categoryId: number) => {
                      // Single category selection (backward compatibility)
                      const updatedFilters = {
                        ...filters,
                        categoryId: categoryId > 0 ? categoryId : undefined,
                        categoryIds: undefined, // Clear multiple selection
                        page: 0,
                      };
                      setFilters(updatedFilters);
                      updateUrlParams(updatedFilters);
                    ...
                    onCategoriesChange={(categoryIds: number[]) => {
                      ...
                      const updatedFilters = {
                        ...
                        categoryId: undefined, // Clear single selection
                        page: 0,
                      };
                      setFilters(updatedFilters);
                      updateUrlParams(updatedFilters);
                  ...
                  <div className="bg-white shadow-1 rounded-lg p-5">
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="text-sm text-gray-6 mb-1 block">
                          Giá tối thiểu
                        </label>
                        <input
                          type="number"
                          ...
                          className="w-full px-3 py-2 border border-gray-3 rounded-lg focus:outline-none focus:border-blue text-sm"
                        />
                      </div>
                      <div>
                        <label className="text-sm text-gray-6 mb-1 block">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      ...
                      <span className="text-dark">
                        {products.length} trong {pagination?.totalCount || 0}
                      </span>{" "}
                      Sản phẩm
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5">
                    <button
                      ...
                      aria-label="nút hiển thị dạng lưới"
                      className={`${
                        productStyle === "grid"
                    ...
                    <button
                      ...
                      aria-label="nút hiển thị dạng danh sách"
                      className={`${
                        productStyle === "list"
                          ? "bg-blue border-blue text-white"
                          : "text-dark bg-gray-1 border-gray-3"
                  ...
                  <div className="bg-white shadow-1 rounded-md p-2">
                    <ul className="flex items-center">
                      ...
                      {generatePaginationNumbers(
                        ...
                        <li key={index}>
                          {pageNum === "..." ? (
                            <span className="flex py-1.5 px-3.5 text-gray-4">
                              ...
                            </span>
                          ) : (
                            <button
