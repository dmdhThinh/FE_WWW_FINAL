...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const isInWishlist = item.id ? wishlistItems.some((wishlistItem) => wishlistItem.id === item.id) : false;
  ...
  const handleItemToWishList = () => {
    // Ensure id exists before adding to wishlist
    if (!item.id) {
      console.error(
        "Cannot add item to wishlist: item.id is required but missing"
      );
      return;
    }
  ...
  const getImageSrc = () => {
    ...
    if (item.imgs?.previews?.[0] && checkImageUrl(item.imgs.previews[0])) {
      return item.imgs.previews[0];
    }
    if (item.imageUrl && checkImageUrl(item.imageUrl)) {
  ...
  // Handle title display with fallback
  const getTitle = () => {
    ...
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        <Image
          ...
          unoptimized={getImageSrc().startsWith('/images/')}
          onError={(e) => {
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <button
            ...
            aria-label={isInWishlist ? "nút xóa khỏi yêu thích" : "nút thêm vào yêu thích"}
            id="favOne"
            ...
            >
              {isInWishlist ? (
                ...
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M8 2.74805L7.283 2.01105C5.6 0.281049 2.514 0.878049 1.4 3.05305C0.877 4.07605 0.759 5.34805 1.714 6.30305L8 12.5861L14.285 6.30305C15.241 5.34905 15.124 4.07605 14.6 3.05305C13.486 0.878049 10.4 0.280049 8.717 2.01005L8 2.74805Z"
                  fill="currentColor"
                />
              ) : (
                ...
                <Image
                  ...
                  src={
                    index < Math.floor(item.rating)
                      ...
                      : "/images/icons/icon-star-empty.svg"
                  }
                  alt="star icon"
                  width={14}
                  height={14}
      ...
      >
        {/* <Link href={`/shop-details/${item.id}`}>{getTitle()}</Link> */}
        <label >{getTitle()}</label>
      </h3>

      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">${priceDisplay.current}</span>
        {priceDisplay.isOnSale && priceDisplay.original && (
