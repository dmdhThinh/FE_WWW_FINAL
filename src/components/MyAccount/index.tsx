...
const MyAccount = () => {
  ...
  // Initialize tab from URL with proper fallback
  ...
  console.log("🏠 MyAccount - Initial state: activeTab =", activeTab);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      console.log("🔍 URL Effect - tab from URL:", tab);
      ...
      if (
        ...
        console.log("✅ Syncing activeTab with URL:", tab);
        setActiveTab(tab);
      } else if (window.location.search.includes("tab=")) {
        console.log("⚠️ Invalid tab param, keeping current:", tab);
      }
    }
  }, [searchParams]);

  // Update URL when active tab changes (but avoid infinite loop)
  useEffect(() => {
    const currentUrl = window.location.pathname + window.location.search;
    if (activeTab === "dashboard") {
      // Should be at /my-account without tab parameter
      if (window.location.search.includes("tab=")) {
        router.replace("/my-account", { scroll: false });
      }
    } else {
      // Should be at /my-account?tab=activeTab
      const expectedUrl = `/my-account?tab=${activeTab}`;
                ...
                <div className="space-y-6">
                  <div className="border-b border-gray-3 pb-6">
                    ...
                    <p className="text-custom-sm text-gray-600">
                      Welcome to your account dashboard. Here you can manage
                      your profile, view orders, and update your account
                      settings.
                    </p>
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start gap-3">
                          <svg
                            className="w-5 h-5 text-blue mt-0.5"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                        ...
                        {userProfile.isAdmin && (
                          <div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
                            ...
                            <p className="text-sm text-purple-700 mb-3">
                              You have administrator privileges. Manage
                              categories, products, orders, and users.
                            </p>
                            <a
                              href="/admin"
              ...
              >
                <div className="bg-white shadow-1 rounded-xl p-4 sm:p-8.5">
                  ...
                  <button
                    ...
                    className="inline-flex justify-center font-medium text-white bg-blue py-3 px-7 rounded-md ease-out duration-200 hover:bg-blue-dark disabled:opacity-50 disabled:cursor-not-allowed"
                  >
