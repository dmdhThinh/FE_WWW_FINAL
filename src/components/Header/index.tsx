...
const Header = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState("0"); // Mặc định là "Tất cả danh mục"
  ...
  const categoryOptions = useAppSelector(selectCategoriesForDropdown);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (userDropdownOpen) {
        ...
        const userDropdown = target.closest(".relative");
        if (!userDropdown) {
  ...
  useEffect(() => {
    dispatch(fetchAllCategories({ page: 0, size: 20 }));
  }, [dispatch]);
  ...
  const handleSubmit = (e) => {
    ...
    // Thêm danh mục nếu không phải "Tất cả danh mục" (giá trị "0")
    if (selectedCategory !== "0") {
      params.set("categoryId", selectedCategory);
    }

    const queryString = params.toString();
    const searchUrl = `/shop-with-sidebar${queryString ? `?${queryString}` : ""}`;
              ...
              <div className="flex items-center gap-5">
                {isAuthenticated && user ? (
                  ...
                  <div className="relative">
                    ...
                    >
                      <div className="flex items-center justify-center w-8 h-8 bg-blue text-white rounded-full text-sm font-medium">
      ...
      <div className="border-t border-gray-3">
        <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
          <div className="flex items-center justify-between">
            ...
            >
              {/* <!-- Main Nav Start --> */}
              {/* Hide navigation when on admin pages */}
